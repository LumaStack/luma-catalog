...
# lumastack/luma-catalog/organization-internal-hq 0.14.0

The internal repository where an organization records what outlives a single project — naming it, creating it, indexing the repositories it reasons about, and rechecking that it is still private.
...
- `policy/what-a-headquarters-holds` (policy) — What belongs in an organization's internal repository, what stays in a project, what goes upstream — and the rule that it stays private.
  - matches: eager

## Offered — open when the work matches

- `policy/the-repository-index` (policy) — Where a headquarters records the repositories it reasons about, why it is one file each, and the rule that keeps it from rotting — store only what cannot be derived.
  - matches: topic:recording or updating a repository in a headquarters

## By request — reached by name or by citation

- `concepts/knowing-without-access` (document) — Why the index records repositories it cannot read, why that is not a security weakness, and why the stance is a default rather than a rule. Read when somebody challenges it.
- `concepts/why-a-headquarters` (document) — What an internal organization repository is for, why it is recommended rather than required, and what would show it was the wrong idea. Read when deciding whether to have one.
- `type_definitions/repository/DEFINITION` (type_definition)
