...
| **led** — the person picks the next cluster each sweep session | attention goes where the interest is, which is the resource that actually runs out | coverage goes uneven, and the dull corners are never what anybody picks | *I know this codebase and I want to steer* |
...
*today I want to look at the transport layer* — is a choice made per sweep
session rather than up front, and it is the right one when the person knows the
system well enough that their instinct about what to read next is better
information than any rule you could write down at the start.
...
one costs a decision per sweep session to buy nothing.
...
So a led sweep does one extra thing at every sweep session: **look at what is
still pending before choosing.** Not to be shamed into the dull corners — to
make the choice with the cost visible, which is the only difference between
...
**What is not allowed is changing it silently**, one convenient sweep session
at a time. That is how an order becomes *whatever seemed next* without anybody
