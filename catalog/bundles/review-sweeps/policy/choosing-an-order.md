...
| **led** — the person picks the next cluster each sitting | attention goes where the interest is, which is the resource that actually runs out | coverage goes uneven, and the dull corners are never what anybody picks | *I know this codebase and I want to steer* |
...
*today I want to look at the transport layer* — is a choice made per sitting
...
picking it while unable to see what is left is how a sweep becomes a tour of the
interesting parts with a completion bar attached.

So a led sweep does one extra thing at every sitting: **look at what is still
...
**Expect the last fifth to be joyless**, and decide about it deliberately rather
than by attrition. Everything nobody wanted to pick is still there, all at once,
and that is the moment a led sweep either finishes or quietly stops. Both are
allowed; being surprised by it is what is not.
...
The backbone is what you return to. Without one, jumping is the whole method and
the sweep should say `led` plainly instead of claiming a structure it abandons
every sitting.
...
deciding to abandon it — and the sweep loses the only record of why its coverage
...
is being led in practice. **The defect is the mismatch, not the steering** — and
the fix is usually to change the recorded order to `led` rather than to go back
to obeying the old one.
