---
type: policy
title: What a sweep session produces
...
# What a sweep session produces

**Everything worth keeping leaves the sweep.** A sweep is backlog: it gets
archived and eventually deleted, and anything parked in a sweep session note as
*we should really…* dies with it.

So the last few minutes of a sweep session are routing, and they are not
optional. A sweep session that ends with six observations in a note has
produced nothing.
...
| --- | --- | --- |
| **this is wrong and I understand it** | fix it now, in this sweep session's branch | the pull request |
...
## Fix what you understood in the sweep session; capture the rest

**The fix that belongs in this sweep session is the one you both just reasoned
...
real observation and it is not sweep session work: taking it on stalls the
sweep on file four for a week, and the sweep is the thing with momentum worth
protecting.

**Watch for `while I'm here`.** It is how a two-file sweep session becomes a
nineteen-file diff nobody can review, including you. The test is whether the
change is one you understood *in this sweep session* — not whether it is small,
and not whether it is obviously correct.

## One pull request per sweep session
...
**Per sweep session is the natural unit** because it is already a coherent
chunk of reasoning: the pull request body is the sweep session note, and it
explains itself.

**Land it before the next sweep session starts.** An unmerged sweep session
means the next one reviews code that is about to change, which is how a sweep
starts reviewing its own uncommitted work.
...
**Route it during the sweep session.** A pile of *to be filed later* items is
filed by nobody: the reasoning that made each one worth capturing is gone
within a day, and what gets written a fortnight later is a shorter, worse
version of it.
...
three more sweep sessions before it can be stated. Write it in the sweep
session as a suspicion, say what would confirm it, and let a later sweep
session settle it.
