...
**If it is missing, stop and say so.** `gh` is **required** — this workflow
...
else in this workflow, and on a shared or managed machine it may be against
...
this workflow does not, such as which files carry a version.
...
**A tag without a release is the failure this workflow exists to prevent**, and
