...
perform: steps, a quiz, and the procedure that drives them.
...
**This is the step of this procedure most likely to be skipped and the one that
...
driving procedure. That table is what stops an agent cheerfully clearing the
...
## 7. Wire up the driving procedure

Template: [a driving procedure](../templates/driving-procedure.md).
...
So: **a fresh session, pointed at the driving procedure and nothing else.** A
...
driving procedure and the template together, instead of guessing at which layer
...
- **wording the agent invented** where the procedure described an outcome rather
...
**Fix the procedure, not the transcript.** Each of those is a gap in what the
driving procedure specified, and fixing it once is what stops the next tutorial
inheriting it.
...
- **`title` matches the step's `# heading`.** The reader sees both — the heading
  the procedure prints, then the one in the body — so a mismatch shows them two
...
lives in it.** Ask, rather than assuming — most people running this procedure are
...
| the order you chose | a question this procedure should have asked and did not |
...
policy that grows every time somebody uses the procedure becomes one nobody reads.
