...
**The only procedure in this bundle that deletes anything, and it is deliberately
awkward to reach.**
...
after, you do not need this procedure**, and [[record-decision]] already did it.
...
reachable from this procedure at all — not with a flag, not with confirmation, not
...
**Absent means this procedure does not run.** Not *fall back to a sensible
...
| `retired` | **Ordinary.** Finished business with no successor. The record is history, and whether history is worth keeping is the actual question this procedure asks |
...
**The *Kept* table is the more useful half**, and the reason to run this procedure
