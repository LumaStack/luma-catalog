...
### Retired decisions go to `archived/`. This procedure does not delete any
...
#### Deleting is a different procedure, and nothing here reaches it
...
**Why the separation is worth the friction.** A procedure with a pruning step
...
**So when they ask to prune during this run, archive it, say the other procedure
...
| **A decision that is plainly wrong now** | Not this procedure's business. Migrate it, and raise superseding it as separate work afterwards |
...
- **No position moved.** For each record, would somebody who followed the entry
  ...
  a decision was changed during a move, which is the failure this whole procedure
