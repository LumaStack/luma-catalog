...
description: 'the-per-session-budget counts words, and words are the smallest part of the bill — prove-what-you-did is 200 words and makes the model run extra commands, quote their output and write longer messages on every turn where a claim is made. The induced cost dwarfs the document and nothing measures it, which is the number that decides whether a guardrail is worth having. Captures broadly everything that could measure it: a fixed task suite run both ways, counterfactual replay, coin-flip A/B over real sessions, transcript audit, usage records, an instrumented read, self-report behind a measurement flag, and cost tiers assigned when a guardrail is written.'
...
**Related:** [[work-items/CATA-0002-guardrails-need-a-switch-and-a-way-to-tell-whether-they-work]] — the switch is a
prerequisite for most of the above, and the fixed task suite is the same
artifact both ideas need.

## Related work

- Born from the idea it replaces: [`what-a-guardrail-costs-in-time`](../../ideas/what-a-guardrail-costs-in-time.md)
