...
description: A sweep advances one file, or a batch of them, and nothing smaller, because slice declares covers as the repository-relative paths read in this slice. Whole files should stay the default, but a review should be able to get much more targeted — a section of prose, a function, a range of lines. Line ranges are cheap and rot; named units survive edits and cost a parser per language; Markdown headings need no parser, so prose could get finer units long before code does. The hard part is coverage arithmetic, where partial becomes a third state and silence must still mean read and nothing found.
...
- Born from the idea it replaces: [`a-slice-could-cover-less-than-a-file`](../../ideas/a-slice-could-cover-less-than-a-file.md)
