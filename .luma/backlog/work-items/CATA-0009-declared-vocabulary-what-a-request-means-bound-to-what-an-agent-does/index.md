...
description: 'A way to declare that a particular request means a particular thing, so an agent resolves it the same way every time instead of improvising. Retitled at migration because three of the five recorded phrases already resolve through the session-manager bundle — so the want is a binding layer, a declaration that this phrase runs that workflow, rather than five new capabilities, and the phrases are the evidence rather than the specification. Two are genuinely uncovered: a situation report, the only one with a specified response, and what do you know to be true.'
workflow_status: captured
rank: 010.0090.000
kind: idea
stage: draft
created: { by: human:luma-founder, at: 2026-08-17T00:00:00Z }
---
...
- Born from the idea it replaces: [`declared-vocabulary-for-requests`](../../ideas/declared-vocabulary-for-requests.md)
