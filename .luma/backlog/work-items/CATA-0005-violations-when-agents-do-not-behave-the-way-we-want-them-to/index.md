...
description: 'A register at .luma/records/violations/, working like incidents and weighing far less, where there should be a lot of them by design — they exist to improve policy and procedure governance rather than to be investigated one at a time. A violation is a breach of policy inside the work; an incident is something that broke or was compromised in the real world. Widening the subject to include what was wanted and never written down adds a third answer that no register requiring a policy_violated field can record. The signal is in the aggregate: one agent skipping one step is noise, and the same step skipped forty times is a procedure that does not work.'
...
- Born from the idea it replaces: [`violations-when-agents-do-not-behave-as-wanted`](../../ideas/violations-when-agents-do-not-behave-as-wanted.md)
