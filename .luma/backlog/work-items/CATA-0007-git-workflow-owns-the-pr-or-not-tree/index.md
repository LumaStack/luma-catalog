...
description: 'The PR-or-not tree belongs to git-workflow rather than git-secrets, because an ordinary change that happens to contain a secret nobody noticed has somebody about to open a pull request with git-secrets unopened — the unattended failure, and the one that actually happens. Written down because the reasoning has now been reached twice from scratch in separate sessions: neither bundle says what it owns at the boundary. Carries the futility argument that a pull request cannot perform a history scrub, the instruction to name the class of publishing mechanisms rather than pull requests alone, and the branch-protection finding that made the correct branch of the tree unreachable on this repository.'
...
- Born from the idea it replaces: [`git-workflow-owns-the-pr-or-not-tree`](../../ideas/git-workflow-owns-the-pr-or-not-tree.md)
