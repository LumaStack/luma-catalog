...
description: 'Pull luma/catalog and luma/project out of luma-types into their own bundles, catalog-ruleset and project-ruleset, each owning everything needed to keep its file the way it should be — the Type Definition, the template, the create/update practice, and the checks a linter enforces. Records how the word ruleset was reached and the bundle taxonomy it instantiates: manager bundles, policy/procedure bundles, and ruleset bundles. What executing it decides is two bundles rather than one, whether a ruleset bundle owns its contract, project-documentation''s fate, and where the foreman linter''s per-kind checks travel.'
...
- Born from the idea it replaces: [`catalog-and-project-become-ruleset-bundles`](../../ideas/catalog-and-project-become-ruleset-bundles.md)
