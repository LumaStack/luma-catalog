...
description: 'An incident is a file today, and incidents are exactly the records that arrive with attachments — logs, a screenshot, a transcript, an export, the report that went to somebody else. There is nowhere to put any of it, while the type already leans on evidence it cannot hold: detected_by names a process, began_at is usually inferred from logs, and the type says outright that an estimate presented as a measurement is the most common lie in an incident record. Proposes a directory per incident holding an index.md and an evidence/ subdirectory, and leaves open whether evidence/ is the name, what listings do with a file that is not a record, and whether every record type wants this.'
workflow_status: captured
rank: 010.0040.000
kind: idea
stage: draft
created: { by: human:luma-founder, at: 2026-09-08T03:10:00Z }
---
...
- Born from the idea it replaces: [`incidents-are-folders-not-files`](../../ideas/incidents-are-folders-not-files.md)
