...
description: 'Every guardrail in model-opus-5-enhancements is adopted on faith. Each one names a behaviour somebody watched cost real time, which is evidence that it was worth writing; nothing is evidence that it works, and the stated goal is zero. Wants two things, the first a prerequisite for the second: a switch that turns one guardrail off without deleting it, most likely an override list in .luma/config/, and a comparison that tells whether the behaviour changed. The hard part is attribution rather than the switch, and the natural signal is whether the user still has to correct the behaviour.'
...
- Born from the idea it replaces: [`guardrails-need-a-switch-and-a-measurement`](../../ideas/guardrails-need-a-switch-and-a-measurement.md)
