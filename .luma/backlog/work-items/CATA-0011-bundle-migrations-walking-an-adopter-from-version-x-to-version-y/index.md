...
description: 'Wanted, not designed: a way to say how to get from version X to version Y, so an adopter several versions behind can be walked forward rather than left to work it out from a changelog. A directory per migration under the bundle, mostly prose an agent follows with scripts alongside where a step is mechanical. The reframe that makes it tractable is that a migration is about the adopter''s content rather than the bundle''s. It belongs in bundle-manager rather than in the format, and what is open is telling no migration needed from migration missing, partial failure, accumulation, and local drift.'
workflow_status: captured
rank: 010.0110.000
kind: idea
stage: draft
created: { by: human:luma-founder, at: 2026-08-19T00:00:00Z }
---
...
- Born from the idea it replaces: [`bundle-migrations`](../../ideas/bundle-migrations.md)
